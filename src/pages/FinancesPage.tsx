...
import { DollarSign, Users, TrendingUp, TrendingDown, Download, PlusCircle, Trash2, Settings } from 'lucide-react';
...
const formatCurrency = (amountInCents: number) => new Intl.NumberFormat('fr-MA', { style: 'currency', currency: 'MAD' }).format(amountInCents / 100);
...
function ConfigForm({ onFinished, existingConfig }: { onFinished: () => void, existingConfig?: FinancialConfig }) {
  ...
  const onSubmit = (values: ConfigFormData) => {
    mutation.mutate({ ...values, basePricePerStudent: values.basePricePerStudent * 100 });
  };
  if (isLoadingCenters) return <div>Loading...</div>;
  return (
    <Form {...form}>
        ...
        <Tabs defaultValue="overview">
          ...
          <TabsContent value="overview" className="space-y-6 pt-4">
            <Card>
              ...
              <CardContent className="pl-2"><div className="h-[350px] w-full">{isLoading ? <Skeleton className="h-full w-full" /> : (<ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" stroke="hsl(var(--muted-foreground))" fontSize={12} tickLine={false} axisLine={false} /><YAxis stroke="hsl(var(--muted-foreground))" fontSize={12} tickLine={false} axisLine={false} tickFormatter={(value) => `${value / 1000}k`} /><Tooltip contentStyle={{ backgroundColor: 'hsl(var(--background))', border: '1px solid hsl(var(--border))', borderRadius: 'var(--radius)' }} formatter={(value: number) => formatCurrency(value * 100)} /><Legend iconType="circle" /><Area type="monotone" dataKey="income" stackId="1" stroke="hsl(var(--primary))" fill="hsl(var(--primary) / 0.2)" name="Income" /><Area type="monotone" dataKey="expenses" stackId="1" stroke="hsl(var(--destructive))" fill="hsl(var(--destructive) / 0.2)" name="Expenses" /></AreaChart></ResponsiveContainer>)}</div></CardContent>
            </Card>
          </TabsContent>
